...
• Comprender de qué se trata el alboroto sobre el Modelo de objetos de documento (DOM)
...
Una página web típica se compone de HTML, CSS y JavaScript.
...
<cabeza>
  <meta content = "nutria marina, niño, cosas" name = "palabras clave">
  <meta content = "¡A veces, las nutrias marinas son increíbles!"
nombre = "descripción">
  <title> Ejemplo </title>
  <link href = "foo.css" rel = "hoja de estilo" />
</head>
...
<cuerpo>
  <div id = "contenedor">
    ...
    <h1> Lo que esta nutria de mar le hizo a este niño te hará
¡LOL! </h1>

    <p class = "bodyText">
      Nulla tristique, justo eget semper viverra,
      massa arcu congue tortor, ut vehicula urna mi
      en lorem. Quisque aliquam molestie dui, en tempor
      turpis porttitor ncop. Enean id interdum urna.
      Curabitur mi ligula, hendrerit en semper sed,
      feugiat a nisi.
    </p>
...
HTML en sí mismo, un poco como Meg Griffin en Padre de familia, es bastante aburrido. Si no sabe quién es Meg y es demasiado vago para buscarla en Google, la Figura 24.2 es una aproximación de su apariencia.

Imagen
FIGURA 24.2

Una interpretación artística de Meg Griffin.
...
.botón de enviar {
  ...
  borde: 5px sólido # 669900;
  color de fondo: # 7BB700;
  relleno: 10px;
  ancho: 150px;
  peso de fuente: 800;
}
...
CONOZCA EL MODELO DE OBJETO DE DOCUMENTO
...
Esta estructura se conoce (nuevamente) como el Modelo de Objetos de Documento. Los amigos simplemente lo llaman DOM. La figura 24.3 muestra una vista muy simplificada de cómo se vería el DOM de nuestro ejemplo anterior:

Imagen
FIGURA 24.3

¡Nuestro DOM para todo el HTML que viste anteriormente se ve así!
...
Cada elemento HTML al que desea acceder tiene un tipo particular asociado, y todos estos tipos se extienden desde la base del nodo que forma todos los nodos, como se muestra en la Figura 24.4.

Imagen
FIGURA 24.4

La disposición de cómo se estructuran los elementos que normalmente vemos.
...
En el navegador, la raíz de su jerarquía es el objeto de ventana que contiene muchas propiedades y métodos que le ayudan a trabajar con su navegador; vea la Figura 24.5.

Imagen
FIGURA 24.5

La ventana es bastante importante en estas partes.

Algunas de las cosas que puede hacer con la ayuda del objeto ventana incluyen acceder a la URL actual, obtener información sobre los marcos de la página, usar el almacenamiento local, ver información sobre su pantalla, manipular la barra de desplazamiento, configurar el texto de la barra de estado, y todo tipo de cosas que son aplicables al contenedor en el que se muestra su página web.

El objeto de documento
Ahora, llegamos al objeto de documento resaltado en la Figura 24.6. Aquí es donde las cosas se ponen interesantes, y también es donde tú y yo nos centraremos gran parte de nuestro tiempo.

Imagen
FIGURA 24.6

El objeto de documento también es algo importante.

El objeto de documento es la puerta de entrada a todos los elementos HTML que componen lo que se muestra. Lo que hay que tener en cuenta (y uno que tiene más sentido a medida que analizamos los capítulos futuros) es que el objeto de documento no representa simplemente una versión de solo lectura del documento HTML. Es una calle de dos vías donde puede leer y manipular su documento a voluntad.
...
Otro aspecto de importación del objeto de documento tiene que ver con los eventos. Entraré en más detalles sobre esto en breve, pero si desea reaccionar a un clic / desplazamiento del mouse, marcar una casilla de verificación, detectar cuándo se presionó una tecla, etc., dependerá de la funcionalidad que proporciona el objeto de documento. escuchando y reaccionando a los eventos.
